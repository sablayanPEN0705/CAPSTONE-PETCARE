...
<head>
    ...
    <style>
        ...
        .ac-value { font-family: var(--font-head); font-size: 36px; font-weight: 800; line-height: 1; margin-bottom: 4px; }
        .ac-label { font-size: 11px; font-weight: 700; opacity: 0.9; text-transform: uppercase; letter-spacing: 0.4px; }
        .ac-sub   { font-size: 11px; opacity: 0.75; margin-top: 2px; }

        /* ── Live alerts strip ── */
        .live-alerts { margin-bottom: 20px; display: flex; flex-direction: column; gap: 10px; }
        .live-alert-row {
        ...
        /* ── Centered delete / clear-all modal ── */
        .del-overlay {
        ...
        <div class="page-body">
            ...
            <div class="notif-header-strip">
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    <?php endif; ?>
                    <?php if ($cnt_all > 0): ?>
                        <!-- Clear All now uses the centered modal -->
                        <button class="btn btn-gray btn-sm"
                                onclick="openClearAllModal()">🗑️ Clear All</button>
                    <?php endif; ?>
                </div>
            </div>
                    ...
                    <div class="notif-item <?= $is_unread ? 'unread' : '' ?> t-<?= $type ?>"
                        ...
                        <div class="notif-content">
                            <div class="notif-title">
                                ...
                                <span class="type-badge tb-<?= $type ?>"><?= $type_lbl ?></span>
                            </div>
                        ...
                        <div class="notif-item-actions" onclick="event.stopPropagation()">
                            ...
                            <button
                                ...
                                onclick="openDeleteModal(
                                    <?= $notif['id'] ?>,
                                    '<?= $filter ?>',
                                    '<?= htmlspecialchars(addslashes($notif['title'] ?? 'this notification'), ENT_QUOTES) ?>'
                                )">
                                🗑️
                            </button>
                        </div>
...
<!-- Detail modal (announcement / general types) -->
...
<div class="del-overlay" id="delSingleOverlay" onclick="if(event.target===this)closeDelSingle()">
    <div class="del-box">
        <div class="del-icon-wrap">🗑️</div>
        <h3>Delete Notification?</h3>
        <p id="delSingleMsg">This notification will be permanently removed.<br>
           <span style="color:#ef4444;font-size:12px;">This action cannot be undone.</span></p>
        <div class="del-actions">
            <button class="btn-keep" onclick="closeDelSingle()">Keep It</button>
            <a id="delSingleConfirmBtn" href="#" class="btn-del-confirm">✕ Yes, Delete</a>
        </div>
    </div>
</div>

<!-- ══ Clear all notifications modal ════════════════════════ -->
<div class="del-overlay" id="delClearAllOverlay" onclick="if(event.target===this)closeDelClearAll()">
    <div class="del-box">
        <div class="del-icon-wrap">🗑️</div>
        <h3>Clear All Notifications?</h3>
        <p>All <?= $cnt_all ?> notification(s) will be permanently removed.<br>
           <span style="color:#ef4444;font-size:12px;">This action cannot be undone.</span></p>
        <div class="del-actions">
            <button class="btn-keep" onclick="closeDelClearAll()">Cancel</button>
            <a href="?clear_all=1" class="btn-del-confirm">✕ Yes, Clear All</a>
        </div>
    </div>
</div>
...
// ── Delete single notification modal ─────────────────────────
function openDeleteModal(id, filter, title) {
    document.getElementById('delSingleMsg').innerHTML =
    ...
    document.getElementById('delSingleConfirmBtn').href =
        'notifications.php?delete=' + id + '&filter=' + encodeURIComponent(filter);
    document.getElementById('delSingleOverlay').classList.add('open');
}

function closeDelSingle() {
    document.getElementById('delSingleOverlay').classList.remove('open');
}

// ── Clear all modal ───────────────────────────────────────────
function openClearAllModal() {
    document.getElementById('delClearAllOverlay').classList.add('open');
}

function closeDelClearAll() {
    document.getElementById('delClearAllOverlay').classList.remove('open');
}
