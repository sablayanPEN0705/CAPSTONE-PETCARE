...
$med_history   = !empty($med_histories) ? $med_histories[0] : null;    $pet_owner     = getRow($conn,  "SELECT * FROM users WHERE id={$view_pet['user_id']}");
}
...
<head>
    ...
    <style>
        ...
        .pet-photo-placeholder {
            ...
            font-size: 56px; box-shadow: var(--shadow);
        }
        .pet-info-box {
        ...
        .med-card-title {
            font-weight: 800; font-size: 14px; color: var(--blue-header);
        ...
        <div class="page-body">
                            ...
                            <div class="med-entry">
                                <div class="med-entry-top">
                                    ...
                                    <div class="med-entry-actions">
                                        ...
                                        <button class="med-action-btn med-delete-btn"
                                                onclick="openDelModal('medication', <?= $med['id'] ?>, <?= $view_id ?>, '<?= htmlspecialchars(addslashes($med['medication_name'])) ?>')">
                                            Del
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="med-entry">
                                <div class="med-entry-top">
                                    ...
                                    <div class="med-entry-actions">
                                        ...
                                        <button class="med-action-btn med-delete-btn"
                                                onclick="openDelModal('vaccine', <?= $vac['id'] ?>, <?= $view_id ?>, '<?= htmlspecialchars(addslashes($vac['vaccine_name'])) ?>')">
                                            Del
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="med-entry">
                                <div class="med-entry-top">
                                    ...
                                    <div class="med-entry-actions">
                                        ...
                                        <button class="med-action-btn med-delete-btn"
                                                onclick="openDelModal('allergy', <?= $alg['id'] ?>, <?= $view_id ?>, '<?= htmlspecialchars(addslashes($alg['allergen'])) ?>')">
                                            Del
                                        </button>
                                    </div>
                                </div>
            ...
            <div style="background:rgba(255,255,255,0.9);border-radius:var(--radius);
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                    ...
                    <button class="btn btn-teal btn-sm"
                            onclick="document.getElementById('addConsultModal').classList.add('open')">+ Add</button>
                </div>
...
<div style="background:rgba(255,255,255,0.9);border-radius:var(--radius);
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
        ...
        <button class="btn btn-teal btn-sm"
                onclick="document.getElementById('editMedHistModal').classList.add('open')">+ Add</button>
    </div>
    ...
    <tr style="border-bottom:1px solid var(--border);<?= $vi%2===0?'background:#fafffe;':'' ?>" <?= $vi >= 5 ? 'class="hidden-row vac-extra"' : '' ?>>
                ...
                <div class="modal">
                    ...
                    <form method="POST">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label>Age (yrs)</label>
                                <input type="text" name="age" value="<?= htmlspecialchars($view_pet['age'] ?? '') ?>">
                            </div>
                            <div class="form-group">
                                <label>Gender</label>
                                <select name="gender">
                ...
                <div class="modal">
                    ...
                    <form method="POST">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label>Prescribed By</label>
                                <input type="text" name="prescribed_by" id="edit_med_prescby">
                            </div>
                            <div class="form-group">
                                <label>Prescription Date</label>
            ...
            <div class="modal-overlay" id="addVacModal">
                <div class="modal">
                    ...
                    <form method="POST">
                        ...
                        <div class="form-group">
                            <label>Notes / Manufacturer</label>
                            <textarea name="notes" rows="2" placeholder="e.g. Nobivac"></textarea>
                        </div>
                        <div class="modal-actions">
            ...
            <div style="background:rgba(255,255,255,0.9);border-radius:var(--radius);
                ...
                <h3 style="font-family:var(--font-head);font-weight:700;font-size:17px;margin-bottom:16px;">Pets</h3>

                <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">
                    <div class="search-box">
                        <span>🔍</span>
                        <input type="text" name="search" placeholder="Search Pet..."
                                        ...
                                        <td>
                                            <div style="display:flex;gap:4px;">
                                                ...
                                                <button class="btn btn-red btn-sm"
                                                        onclick="openDelModal('pet', <?= $pet['id'] ?>, 0, '<?= htmlspecialchars(addslashes($pet['name'])) ?>')">
                                                    Delete
                                                </button>
                                            </div>
                                        </td>
...
<div class="modal-overlay" id="editMedHistModal">
    <div class="modal" style="max-width:500px;">
        ...
        <h3 class="modal-title">🏥 Add Medical History</h3>
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="past_illnesses"
            ...
            <div class="form-group">
                ...
                <input type="text" name="injuries_surgeries"
...
<!-- Hidden delete forms — one per record type -->
<form id="delFormMedication"   method="POST" style="display:none;">
    <input type="hidden" name="action"  value="delete_medication">
    <input type="hidden" name="med_id"  id="del_med_id">
    <input type="hidden" name="pet_id"  id="del_med_pid">
</form>
<form id="delFormVaccine"      method="POST" style="display:none;">
    <input type="hidden" name="action"  value="delete_vaccine">
    <input type="hidden" name="vac_id"  id="del_vac_id">
    <input type="hidden" name="pet_id"  id="del_vac_pid">
</form>
<form id="delFormAllergy"      method="POST" style="display:none;">
    <input type="hidden" name="action"      value="delete_allergy">
    <input type="hidden" name="allergy_id"  id="del_allergy_id">
    <input type="hidden" name="pet_id"      id="del_allergy_pid">
</form>
<form id="delFormConsultation" method="POST" style="display:none;">
    <input type="hidden" name="action"     value="delete_consultation">
    <input type="hidden" name="consult_id" id="del_consult_id">
    <input type="hidden" name="pet_id"     id="del_consult_pid">
</form>
<!-- Pet delete goes via GET -->
...
const DEL_CONFIG = {
    ...
    pet:          { icon:'🐾', title:'Delete Pet Record?',   form:null,                  idField:null,             pidField:null },
...
function openDelModal(type, id, pid, label) {
    ...
    document.getElementById('delMsg').innerHTML =
        `<strong>${label}</strong> will be permanently deleted.<br>
...
function submitDelete() {
    ...
    if (_delType === 'pet') {
        const petId = document.getElementById('delConfirmBtn').dataset.petId;
        window.location.href = 'pet_records.php?delete_pet=' + petId;
    } else {
...
function toggleRows(cls, btn) {
    ...
    btn.textContent = isHidden
        ? '▲ Show Less'
        : btn.textContent.replace('▲ Show Less', '').trim() || btn.textContent;
    if (!isHidden) {
