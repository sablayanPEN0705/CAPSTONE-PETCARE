...
<head>
    ...
    <style>
        ...
        .inv-header h3 { font-family: var(--font-head); font-size: 17px; font-weight: 700; }
        ...
        .fpill:hover, .fpill.on { background: var(--teal); color: #fff; border-color: var(--teal); }
        .stock-bar-wrap {
        ...
        .prod-thumb {
            ...
            background: #f4fffe; display: flex; align-items: center;
            justify-content: center; font-size: 20px;
        }
        .restock-form { display: flex; gap: 4px; align-items: center; }
        .rq-input {
            width: 52px; padding: 3px 6px; border: 1.5px solid var(--border);
            border-radius: 4px; font-size: 12px; text-align: center; outline: none;
        }
        .rq-input:focus { border-color: var(--teal); }
        .alert-banners { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 20px; }
        .ab {
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex;align-items:center;gap:4px;">
                                        ...
                                        <span style="font-weight:700;font-size:13px;"><?= $p['quantity'] ?></span>
                                ...
                                <td>
                                    <div style="display:flex;gap:4px;">
                                        ...
                                        <!-- Delete uses centered modal instead of confirm() -->
                                        <button class="btn btn-red btn-sm"
                                                onclick="openDelModal(<?= $p['id'] ?>, '<?= htmlspecialchars(addslashes($p['name'])) ?>')">
                                            Delete
                                        </button>
                                    </div>
                                </td>
    ...
    <div class="modal" style="max-width:500px;">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" placeholder="e.g. Groom and Bloom Shampoo" required>
            </div>
            <div class="form-row">
                <div class="form-group">
            ...
            <div style="background:#e0f7fa;border-radius:var(--radius-sm);
                        padding:10px 14px;font-size:12px;color:var(--teal-dark);margin-bottom:14px;">
...
<div class="del-overlay" id="delOverlay" onclick="if(event.target===this)closeDelModal()">
    <div class="del-box">
        ...
        <p id="delMsg">This product will be permanently deleted.<br>
...
function openDelModal(id, name) {
    document.getElementById('delMsg').innerHTML =
        '<strong>' + name + '</strong> will be permanently deleted.<br>' +
        '<span style="color:#ef4444;font-size:12px;">This action cannot be undone.</span>';
    document.getElementById('delConfirmBtn').href = 'inventory.php?delete=' + id;
...
function getTableData() {
    ...
    table.querySelectorAll('thead th').forEach((th, i) => {
        ...
        if (t === 'Image' || t === 'Action') { skipCols.push(i); }
        else { headers.push(t); }
    });
    const rows = [];
    table.querySelectorAll('tbody tr').forEach(tr => {
        ...
        tr.querySelectorAll('td').forEach((td, i) => {
            if (!skipCols.includes(i)) {
                row.push(getCellText(td, headers[row.length]));
            }
        });
        rows.push(row);
    });
...
function doExportPDF() {
    ...
    doc.setTextColor(0);
    doc.autoTable({
        startY: 88, head: [data.headers], body: data.rows,
