...
// File: admin/settings.php  (with activity logging)
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_account') {
        ...
        // ── LOG ──
        logActivity($conn, $admin_id, 'admin', 'update_profile', 'settings',
            "Admin updated own profile (name: $name, email: $email).",
            ['email' => $email, 'has_photo' => !empty($psql)]
        );
    ...
    if ($action === 'change_password') {
        ...
        if (!password_verify($cur, $admin['password'])) {
            logActivity($conn, $admin_id, 'admin', 'change_password_failed', 'settings',
                'Admin password change failed: incorrect current password.');
        ...
        logActivity($conn, $admin_id, 'admin', 'change_password', 'settings',
    ...
    if ($action === 'add_staff') {
        ...
        logActivity($conn, $admin_id, 'admin', 'add_staff', 'staff',
            "Created new staff account: $sname ($semail), position: $spos.",
            ['new_staff_id' => $new_staff_id, 'position' => $spos]
        );
    ...
    if ($action === 'edit_staff') {
        ...
        $detail = "Updated staff account ID $sid ($sname / $semail), position: $spos, status: $sstatus.";
        if ($pass_reset) $detail .= ' Password was also reset.';
        logActivity($conn, $admin_id, 'admin', 'edit_staff', 'staff', $detail,
            ['staff_id' => $sid, 'status' => $sstatus, 'password_reset' => $pass_reset]
        );
    ...
    if ($action === 'delete_staff') {
        $sid = (int)$_POST['staff_id'];
        $staff_row = getRow($conn, "SELECT name, email FROM users WHERE id=$sid AND role='staff'");
        mysqli_query($conn, "DELETE FROM users WHERE id=$sid AND role='staff'");

        $sname = $staff_row ? $staff_row['name'] : "ID $sid";
        $semail = $staff_row ? $staff_row['email'] : '';
        logActivity($conn, $admin_id, 'admin', 'delete_staff', 'staff',
            "Deleted staff account: $sname ($semail).",
            ['deleted_staff_id' => $sid]
        );
    ...
    if ($action === 'toggle_staff_status') {
        ...
        $staff_row = getRow($conn, "SELECT name FROM users WHERE id=$sid");
        $sname = $staff_row['name'] ?? "ID $sid";

        logActivity($conn, $admin_id, 'admin', 'toggle_staff_status', 'staff',
            "Changed staff $sname (ID $sid) status from '$cur_st' to '$new_st'.",
            ['staff_id' => $sid, 'old_status' => $cur_st, 'new_status' => $new_st]
        );
...
$staff_list = getRows($conn, "SELECT * FROM users WHERE role='staff' ORDER BY name ASC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .sc { background:rgba(255,255,255,.9); border-radius:var(--radius); padding:28px 32px; box-shadow:var(--shadow); }
        .sc h3 { font-family:var(--font-head); font-size:18px; font-weight:800; margin-bottom:24px; display:flex; align-items:center; gap:8px; }
        .av-wrap { display:flex; align-items:center; gap:20px; margin-bottom:24px; padding-bottom:20px; border-bottom:1px solid var(--border); }
        .av-circle { width:80px; height:80px; border-radius:50%; overflow:hidden; flex-shrink:0; background:linear-gradient(135deg,var(--teal),#0097a7); display:flex; align-items:center; justify-content:center; font-size:30px; color:#fff; font-weight:700; }
        ...
        .tsl { position:absolute; cursor:pointer; inset:0; background:#ccc; border-radius:24px; transition:var(--transition); }
        .tsl::before { content:''; position:absolute; height:18px; width:18px; left:3px; bottom:3px; background:#fff; border-radius:50%; transition:var(--transition); }
        .tog input:checked + .tsl { background:var(--teal); }
        .tog input:checked + .tsl::before { transform:translateX(20px); }
        .nr { display:flex; align-items:center; justify-content:space-between; padding:13px 0; border-bottom:1px solid var(--border); }
        ...
        .staff-card { background:#fff; border-radius:var(--radius); padding:18px 20px; box-shadow:var(--shadow); display:flex; align-items:center; gap:16px; margin-bottom:14px; border:1.5px solid var(--border); transition:var(--transition); }
        .staff-card:hover { border-color:var(--teal-light); }
        .staff-avatar { width:52px; height:52px; border-radius:50%; background:linear-gradient(135deg,var(--teal),#0097a7); display:flex; align-items:center; justify-content:center; font-size:22px; color:#fff; font-weight:700; flex-shrink:0; overflow:hidden; }
                ...
                <div class="sc">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group"><label>Full Name *</label>
                            <input type="text" name="name" value="<?= htmlspecialchars($admin['name']) ?>" required></div>
                        <div class="form-group"><label>Address</label>
                            <input type="text" name="address" value="<?= htmlspecialchars($admin['address'] ?? '') ?>"></div>
                        <div class="form-row">
                            <div class="form-group"><label>Contact No.</label>
                                <input type="text" name="contact" value="<?= htmlspecialchars($admin['contact_no'] ?? '') ?>"></div>
                            <div class="form-group"><label>Email *</label>
                                <input type="email" name="email" value="<?= htmlspecialchars($admin['email']) ?>" required></div>
                        </div>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <?php foreach([
                            ...
                            <div style="background:rgba(0,188,212,.07);border-radius:var(--radius-sm);padding:14px;border:1px solid var(--teal-light);">
                                <div style="font-size:22px;margin-bottom:6px;"><?= $ib[0] ?></div>
                                <div style="font-size:11px;color:var(--text-light);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;"><?= $ib[1] ?></div>
                    ...
                    <?php else: ?>
                    <h3>⚙️ Settings</h3>
                    <p style="font-size:13px;color:var(--text-light);margin-bottom:20px;">Select a settings category below.</p>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                        <?php foreach([
                            ['account','👤','Account Settings','Edit profile and password'],
                            ['staff',  '👥','Staff Management','Manage clinic staff accounts'],
                            ...
                            <a href="?panel=<?= $ov[0] ?>"
                               style="text-decoration:none;background:rgba(255,255,255,.8);border:1.5px solid var(--border);border-radius:var(--radius);padding:16px;transition:var(--transition);display:block;"
                        ...
                        <a href="activity_log.php"
                           ...
                           onmouseout="this.style.borderColor='var(--border)'">
                            ...
                            <div style="font-size:12px;color:var(--text-light);">View all system activity & audit trail</div>
                        </a>
                    </div>
                    <?php endif; ?>
...
<div class="modal-overlay" id="addStaffModal">
    <div class="modal" style="max-width:480px;">
        <button class="modal-close" onclick="document.getElementById('addStaffModal').classList.remove('open')">×</button>
        <h3 class="modal-title">👥 Add Staff Account</h3>
        <form method="POST">
            <input type="hidden" name="action" value="add_staff">
            <div class="form-group"><label>Full Name *</label>
                <input type="text" name="staff_name" placeholder="e.g. Dr. Ann Lawrence" required></div>
            <div class="form-row">
                <div class="form-group"><label>Position / Role</label>
                    <select name="staff_position">
                        <option>Veterinarian</option><option>Clinic Staff</option>
                        <option>Groomer</option><option>Vet Assistant</option><option>Multitasker</option>
                    </select>
                </div>
                <div class="form-group"><label>Contact No.</label>
                    <input type="text" name="staff_contact"></div>
            </div>
            <div class="form-group"><label>Email Address *</label>
                <input type="email" name="staff_email" required></div>
            <div class="form-group"><label>Password *</label>
                <input type="password" name="staff_password" placeholder="Min. 6 characters" required></div>
            <div style="background:#e0f7fa;border-radius:var(--radius-sm);padding:10px 14px;font-size:12px;color:var(--teal-dark);margin-bottom:14px;">
                💡 Share login credentials with the staff member after creation.
            </div>
            <div class="modal-actions">
                <button type="button" class="btn btn-gray" onclick="document.getElementById('addStaffModal').classList.remove('open')">Cancel</button>
...
<div class="modal-overlay" id="editStaffModal">
    <div class="modal" style="max-width:480px;">
        <button class="modal-close" onclick="document.getElementById('editStaffModal').classList.remove('open')">×</button>
        <h3 class="modal-title">✏️ Edit Staff Account</h3>
        <form method="POST">
            <input type="hidden" name="action"   value="edit_staff">
            <input type="hidden" name="staff_id" id="es_id">
            <div class="form-group"><label>Full Name *</label>
                <input type="text" name="staff_name" id="es_name" required></div>
            <div class="form-row">
                ...
                <div class="form-group"><label>Contact No.</label>
                    <input type="text" name="staff_contact" id="es_contact"></div>
            </div>
            <div class="form-row">
                <div class="form-group"><label>Email *</label>
                    <input type="email" name="staff_email" id="es_email" required></div>
            ...
            <div class="form-group">
                <label>New Password <span style="color:var(--text-light);font-weight:400;">(leave blank to keep)</span></label>
                <input type="password" name="staff_new_password" placeholder="Min. 6 characters">
            </div>
            <div class="modal-actions">
                <button type="button" class="btn btn-gray" onclick="document.getElementById('editStaffModal').classList.remove('open')">Cancel</button>
                <button type="submit" class="btn btn-teal">Save Changes</button>
            </div>
        </form>
    </div>
