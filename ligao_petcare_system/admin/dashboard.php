...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_promo') {
        ...
        if ($title) {
            mysqli_query($conn,
                "INSERT INTO promotions (title, description, icon, link_label, link_url)
                 VALUES ('$title','$desc','$icon','$llabel','$lurl')");
        }
        ...
        mysqli_query($conn,
            "UPDATE promotions
             SET title='$title', description='$desc', icon='$icon',
                 link_label='$llabel', link_url='$lurl'
             WHERE id=$pid");
        redirect('dashboard.php?promo_success=1');
    }
...
<head>
    ...
    <style>
        ...
        /* Preview banner — mirrors user dashboard look */
        ...
        .promo-preview-slide {
            display: none; width: 100%; padding: 20px 28px;
        ...
        .pps-title { font-family: var(--font-head); font-size: 19px; font-weight: 800; margin-bottom: 4px; text-shadow: 0 2px 6px rgba(0,0,0,0.18); }
        .pps-desc  { font-size: 13px; opacity: 0.88; margin-bottom: 10px; }
                    ...
                    <?php else: ?>
                        <?php foreach ($announcements as $ann): ?>
                            <div class="ann-entry">
                                ...
                                <div class="ann-entry-title"><?= htmlspecialchars($ann['title']) ?></div>
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') closePromoDelModal();
});
