...
<div class="app-wrapper">
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <?php if($billing): ?>
            <div class="modal-overlay open" id="billingModal">
                <div class="modal" style="max-width:480px;">
                    <button class="modal-close" onclick="window.location='appointments.php'">×</button>
                    <h3 class="modal-title">Billing Summary Details</h3>
                    <div style="font-size:13px;color:var(--text-mid);margin-bottom:16px;">
                        <p><strong>Pet:</strong> <?= htmlspecialchars($billing['pet_name']??'—') ?></p>
                        <p><strong>Species/Breed:</strong> <?= ucfirst($billing['species']??'') ?>/<?= htmlspecialchars($billing['breed']??'—') ?></p>
                        <p><strong>Owner:</strong> <?= htmlspecialchars($billing['owner_name']) ?></p>
                        <p><strong>Date:</strong> <?= formatDate($billing['transaction_date']) ?></p>
                    </div>
                    <p style="font-size:13px;font-weight:700;margin-bottom:6px;">Service/Product availed:</p>
                    <?php if(!empty($billing['items'])): ?>
                        <table class="billing-table"><tbody>
                            <?php foreach($billing['items'] as $item): ?>
                                <tr><td><?= htmlspecialchars($item['item_name']) ?></td><td><?= formatPeso($item['price']) ?></td></tr>
                            <?php endforeach; ?>
                            <tr class="billing-total"><td>Total:</td><td><?= formatPeso($billing['total_amount']) ?></td></tr>
                        </tbody></table>
                    <?php else: ?>
                        <p style="color:var(--text-light);font-size:13px;">No billing items recorded.</p>
                        <p style="font-weight:800;margin-top:8px;">Total: <?= formatPeso($billing['total_amount']) ?></p>
                    <?php endif; ?>
                    <div class="modal-actions"><a href="appointments.php" class="btn btn-teal">Close</a></div>
                </div>
            </div>
            <?php endif; ?>
                                ...
                                <tbody>
                                    <?php foreach($history as $h): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php $has_bill=getRow($conn,"SELECT id FROM transactions WHERE appointment_id={$h['id']}"); ?>
                                                <?php if($has_bill): ?><a href="appointments.php?billing=<?= $h['id'] ?>" style="color:var(--teal-dark);font-weight:700;font-size:12px;">View Details...</a><?php else: ?><span style="color:var(--text-light);font-size:12px;">—</span><?php endif; ?>
                                            </td>
