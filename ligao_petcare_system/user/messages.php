    ...
    if (($msg || $attachment) && $to) {
        ...
        mysqli_query($conn,
            "INSERT INTO messages (sender_id, receiver_id, message, attachment)
             VALUES ($user_id, $to, '$msg', '$attach_col')");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_msg_id'])) {
    ...
    if ($action === 'unsend') {
        mysqli_query($conn, "DELETE FROM messages WHERE id=$del_id AND sender_id=$user_id");
    } elseif ($action === 'delete') {
        mysqli_query($conn, "UPDATE messages SET deleted_by_user=1 WHERE id=$del_id AND receiver_id=$user_id");
    }
    header("Location: messages.php?staff=$redirect_to");
    exit();
}
// ── Mark messages from this staff as read ────────────────────
if ($chat_with) {    mysqli_query($conn,
...
function submitMessage() {
    ...
    const file  = document.getElementById('fileInput');
    return (input && input.value.trim().length > 0) || (file && file.files.length > 0);
}
